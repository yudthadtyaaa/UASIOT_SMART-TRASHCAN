...
// Pin definitions
const int trigPin = D6;   // Pin Trig sensor ultrasonik
const int echoPin = D7;   // Pin Echo sensor ultrasonik
const int servoPin = D1;  // Pin kontrol servo    // Pin kontrol LED
...
char msg[MSG_BUFFER_SIZE];  // Track the state of the servo
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println();
  if(strcmp(topic, "ultrasonic/akses")== 0 ){
    if(data == "open"){
       myservo.write(90); // Move servo to open position if distance is <= 5 cm // Update the state
      client.publish("ultrasonic/akses", "akses manual terbuka");
    }else if(data == "close"){
       myservo.write(0);  // Move servo to closed position if distance is > 5 cm  // Update the state
      client.publish("ultrasonic/akses", "akses manual tertutup");
    }
  }
...
void setup() {
  ...
  pinMode(echoPin, INPUT);  // Initialize ultrasonic sensor Echo pin    // Initialize LED pin
